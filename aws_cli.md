...
The {{site.data.keyword.cephaas_short}} endpoint must be sourced by using the `--endpoint-url` option, and can't be set in the credentials file.
...
If the default region in the `~/.aws/config` file corresponds to the same location as the chosen endpoint, then bucket creation is straightforward.
...
It's also possible to set a time-to-live for the URL in seconds (the default is 3600):
