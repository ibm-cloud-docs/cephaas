...
# Enabling event notifications for IBM Fusion as a service
{: #event-notifications-events}

As an administrator of IBM Fusion as a service, you might want to send notifications of events in IBM Fusion as a service to other users, or human destinations, by using email, SMS, or other supported delivery channels. Additionally, you might want to send these notifications of events to other applications to build logic by using event-driven programming using webhooks, for example. This is made possible by the integration between IBM Fusion as a service and {{site.data.keyword.en_full}}.
{: shortdesc}

To send information to {{site.data.keyword.en_short}}, you must connect your IBM Fusion as a service instance to {{site.data.keyword.en_short}}. For more information about working with {{site.data.keyword.en_short}}, see [Getting started with {{site.data.keyword.en_short}}](/docs/event-notifications?topic=event-notifications-getting-started).

## How events are collected and sent by IBM Fusion as a service
{: #event-notifications-how}

When an event of interest takes place in your IBM Fusion as a service instance, IBM Fusion as a service communicates with a connected {{site.data.keyword.en_short}} instance to forward a notification to a [supported destination](/docs/event-notifications?topic=event-notifications-en-destination).


## Events for IBM Fusion as a service
{: #event-notifications-list}

The following tables list the IBM Fusion as a service events.
...
Events that are generated by an instance of IBM Fusion as a service can be forwarded to an {{site.data.keyword.en_short}} service instance that is available in the same account. You can configure only one {{site.data.keyword.secrets-manager_short}} instance to one {{site.data.keyword.en_short}} service instance.

### Connecting to {{site.data.keyword.en_short}} in the console
{: #event-notifications-enable-ui}

Before you can enable notifications for IBM Fusion as a service, be sure that you have an [{{site.data.keyword.en_short}} service instance](/catalog/services/event-notifications){: external} that is in the same account as your IBM Fusion as a service instance. Then, you can use the **Settings > Event Notifications** section in the IBM Fusion as a service UI to connect the services.


1. In the console, click the **Navigation Menu** icon ![Navigation Menu icon](../icons/icon_hamburger.svg) **> Resource list**.
2. Select your instance of IBM Fusion as a service.
3. In the IBM Fusion as a service navigation, click **Settings**.
...
6. Select the resource group and {{site.data.keyword.en_short}} service instance that you want to connect.
    ...
    If an IAM authorization between IBM Fusion as a service and {{site.data.keyword.en_short}} doesn't exist in your account, a dialog is displayed. Follow the prompts to grant access between the services.

    1. To grant access between IBM Fusion as a service and {{site.data.keyword.en_short}}, click **Authorize**.
...
After you enable notifications for IBM Fusion as a service, create topics and subscriptions in {{site.data.keyword.en_short}} so that alerts can be forwarded and delivered to your selected destinations.
...
An email from {{site.data.keyword.cloud_notm}} that contains information about IBM Fusion as a service event resembles the following example:
...
Successful events that are generated by IBM Fusion as a service contain various fields that help you to identify the source and details of an event.

Event notifications from IBM Fusion as a service contain only metadata properties, such as names or identifiers of resources. Sensitive data, for example API keys or passwords, are not included in generated events.
{: note}
...
| `source` | This is the identifier of the event producer (i.e. the IBM Fusion as a service CRN). |
